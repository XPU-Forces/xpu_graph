...
class LayerNorm2(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states.to(torch.float32)
        mean = hidden_states.mean(-1, keepdim=True)
        variance = torch.var(hidden_states, dim=-1, keepdim=True, unbiased=False)
        hidden_states = (hidden_states - mean) * torch.rsqrt(variance + self.variance_epsilon)
        return (hidden_states * self.weight + self.bias).to(input_dtype)

    def extra_repr(self):
        return f"{tuple(self.weight.shape)}, eps={self.variance_epsilon}"


fn0 = LayerNorm1(hidden_size=(10,)).npu()
fn1 = LayerNorm2(hidden_size=(10,)).half().npu()
...
class TestLayerNorm:
    ...
    def test_layernorm_patterns(self, caplog, pattern_func):
        ...
        assert "Pattern.FastLayerNorm changed graph" in caplog.text


if __name__ == "__main__":
    xpu_graph_backend = xpu_graph.npu_compiler(
        opt_level=OptLevel.level2, vendor_compiler_config={"compiler": "ge", "mode": "reduce-overhead"}, debug=True
    )
