...
def test_skip_pattern(caplog):
    ...
    assert "Skip OptLevel.level1 pattern FoldAdd0" in caplog.text
    assert "Skip OptLevel.level1 pattern FoldMul1" in caplog.text


def test_include_pattern(caplog):
    with need_xpu_graph_logs():
        config = XpuGraphConfig(
            is_training=False, opt_level=OptLevel.level0, include_patterns=["FoldAdd0", "FoldMul1", "Dummy"], debug=True
        )
        backend = XpuGraph(config)
    ...
    assert has_fold_add0
    assert not has_fold_sub0
    assert has_fold_mul1
    assert not has_fold_div1

    assert "Add OptLevel.level1 pattern FoldAdd0" in caplog.text
    assert "Add OptLevel.level1 pattern FoldMul1" in caplog.text
