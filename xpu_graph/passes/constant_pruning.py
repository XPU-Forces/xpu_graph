...
class ConstantPruning(Optimizer):
    ...
    def process(self, gm: torch.fx.GraphModule) -> bool:
        ...
        all_attribute_names = set(dict(gm.named_parameters(recurse=False)).keys()).union(
            dict(gm.named_buffers(recurse=False)).keys(), gm.__dict__.keys()
        )
            ...
            if name.startswith(_ALL_CONSTANT_PREFIXES):
                if name not in alive_constants:
                    try:
                        ...
                        changed = True
                        logger.debug(f"[ConstantPruning] Pruned unused constant attribute: {name}")
                    except AttributeError as e:
                        logger.warning(f"[ConstantPruning] Failed to delete {name}: {e.__class__.__name__} - {e}")
