...
class FusedLayerNorm(Pattern):
    ...
    def process(self, graph_module: fx.GraphModule) -> bool:
        ...
        for node in reversed(graph_module.graph.nodes):
            # Note: This pattern does not fuse residuals, so F.layer_norm is suitable rather than mlu_ops.fused_layer_norm
            ...
            with graph_module.graph.inserting_before(node):
                layer_norm_node = graph_module.graph.call_function(
                    torch.nn.functional.layer_norm, args=(input, get_shape(input)[-1:], weight, bias, eps)
                )
